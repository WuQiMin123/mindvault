...
个人知识库系统 — 发链接/写笔记 → AI 自动处理 → 标签归档 → 搜索检索。

## 功能

- **内容摄入** — 粘贴链接、纯文本、B站视频（字幕自动提取），多种方式快速保存
- **AI 辅助** — 自动生成摘要、智能打标签（基于 DeepSeek API）
- **标签管理** — 自定义标签，多对多关联，标签内聚合总结
- **全文搜索** — 关键词搜索 + 标签组合筛选，分页展示
- **浏览器插件** — 选中网页文本一键保存，B站视频字幕提取
...
│   └── app/
│       ├── api/v1/           # REST API 路由
│       │   ├── ingest.py     #   POST /ingest/link|text|bookmark
│       │   ├── links.py      #   GET/PATCH /links
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/v1/ingest/link` | 提交链接（后台抓取） |
| POST | `/api/v1/ingest/bookmark` | 快速暂存链接 |
